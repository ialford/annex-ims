...
RSpec.describe Batch do
  ...
  def create_item_with_tray_and_shelf(tray_barcode:, shelf_barcode:)
    shelf = Shelf.where(barcode: shelf_barcode).take || create(:shelf, barcode: shelf_barcode)
    tray = Tray.where(barcode: tray_barcode).take || create(:tray, barcode: tray_barcode, shelf: shelf)
